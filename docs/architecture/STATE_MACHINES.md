...
flowchart LR
    ...
    task["Runtime Task\nowner: Scheduler"]
    agentGraph["Agent Graph State\nowner: Agent Worker"]
    action["ActionTransaction\nowner: Action Executor"]

    incident -->|"ScheduleTask command / TaskScheduled event"| task
    task -->|"leased execution / checkpoint event"| agentGraph
    agentGraph -->|"ActionProposal command"| action
    action -->|"ActionResult event"| agentGraph
    agentGraph -->|"InvestigationResult event"| incident
