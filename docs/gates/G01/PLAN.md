...
- 已暴露的服务器密码必须轮换；Bailian key 应重新签发。只记录 secret version/creation time，不记录值、可逆 hash 或 suffix。
- LangSmith key 通过交互式 secret CLI 直接写入 SOPS，不再经过 plaintext handoff file。
...
- SOPS + Age、SSH key/host pin、secret bootstrap CLI 和 `envs.txt` migration/deletion。
...
- `envs.txt` 删除且 ignore；旧 credential 已轮换。
...
1. Secret migration、credential rotation 和 public-repo scan。
