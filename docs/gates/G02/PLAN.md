...
containerd may register an imported Docker Hub reference under the `index.docker.io` alias while
  ...
  canonical-owner cell; C-G02-002 repaired that single request-contract root cause. A-G02-002 then
  exposed the baseline-agent external TCP probe seam; C-G02-003 and A-G02-003 are its separate
  forward corrective and Gate-attempt paths under AMD-0006.
...
| C-G02-003 root-cause fix plus targeted proof | 0h35 |
| A-G02-003 attempt setup, excluding Gate Eval | 0h15 |
| Remaining A-G02-003 work including Gate Eval | **2h48** |
...
| C-G02-003 Baseline LangSmith TCP Egress Probe Corrective | Falsify the client-abstraction hypothesis, then change only the external network assertion if confirmed | I-0024, terminal A-G02-002 | 0h35 | Same-pod TCP events prove the expected allow and two policy rejects without Gate N or a wall-clock kill timeout. | No L2; repairs V-G02-009 seam |
| A-G02-003 Post-Egress Unified Candidate Gate Attempt | Freeze the post-C-G02-003 candidate and run all fourteen frozen phases in EVAL-G02-028 | Frozen owners plus C-G02-003 | 0h15 excluding Gate Eval | The candidate passes using only frozen runners and immutable evidence. | No new L2 |
...
AMD-0006 records EVAL-G02-026 as a terminal failed attempt and creates C-G02-003/A-G02-003 without
reopening prior work. A network-only authorization assertion must terminate on a concrete transport
allow or policy-reject event; an application-layer downloader with no observable TCP progress is
not an acceptable proxy. The corrective begins with a same-pod falsification seam and stops before
implementation if transport-only TCP also fails. This changes no validation N, permission,
quality/performance metric, failure semantic, destructive rule, model route, token/cost ceiling, or
the prohibition on preset orchestration kill timeouts.
...
| V-G02-002 | Double SHA/binding; excludes false candidate and stale inheritance | L2; unified candidate only | 0 | 1 | One-time bootstrap | I-0016 | `g02.candidate_binding` | `candidate_non_evidence_descendant.json` | `EVAL-G02-028/.../preflight-candidate-binding/summary.json` |
| V-G02-003 | Manifest debt; excludes fail-late remote work | L2; full manifest set exists only at Gate | 0 | 13 | All 9 G01 + 4 G02 manifests required | I-0016 | `g02.manifest_debt` | `manifest_open_evidence.json` | `EVAL-G02-028/.../preflight-manifests/manifest-debt.json` |
...
| V-G02-009 | Access matrix; excludes Agent/developer/evaluator/controller privilege drift | L3; four policy identities then 60 live cells | 4 | 60 | All four identities required | I-0024 | `g02.access_matrix` | `access_wrong_allow.yaml` | I-0018 policy simulation plus EVAL-G02-028 Gate matrix |
| V-G02-010 | G01 six-stage spans; excludes missing correlated stage evidence | L2; complete stack required | 0 | 6 | Six stages are exhaustive | I-0024 | `g02.stage_matrix` | `trace_missing_stage.json` | EVAL-G02-028 Gate six-stage matrix |
| V-G02-011 | G01 all-surface canary; excludes Secret/PII leakage | L3; four new writers then 22 live cells | 4 | 22 | All four writers required | I-0024 | `g02.canary_matrix` | `canary_leaked_artifact.json` | I-0018 writer proof plus EVAL-G02-028 Gate matrix |
...
- A-G02-002/EVAL-G02-026 is immutable failed history. C-G02-003 owns only the expected-allow TCP
  seam; A-G02-003/EVAL-G02-028 is the separate next unified-candidate attempt. Final artifact paths
  move forward to EVAL-G02-028; EVAL-G02-024 and EVAL-G02-026 are never rebound or overwritten.
